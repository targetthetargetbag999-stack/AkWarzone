...
class Match {
  ...
  addPlayer(socket, username) {
    ...
    this.players[socket.id] = {
      ...
      rotY: 0, pitch: 0,
      hp: PLAYER.MAX_HP, shield: 0,
      alive: true, weapon: null, ammo: 0,
      kills: 0, lastShot: 0, reloading: false, reloadEnd: 0,
    };
    this.kills[socket.id] = 0;
    socket.join(this.id);
    socket.emit('matchJoined', {
      matchId: this.id, playerId: socket.id, state: this.state,
      players: this._publicPlayers(), loot: this.loot, zone: this.zone.getState(),
    });
  ...
  _startMatch() {
    ...
    this.io.to(this.id).emit('matchStart', {
      loot: this.loot, zone: this.zone.getState(), players: this._publicPlayers(),
    });
  ...
  handleShoot(socketId, data) {
    ...
    if (shooter.weapon === 'SHOTGUN') {
      for (let i = 0; i < wDef.pellets; i++) this._raycast(socketId, data, wDef, wDef.damage);
    } else {
    ...
    this.io.to(this.id).emit('shotFired', {
      id: socketId, ox: data.ox, oy: data.oy, oz: data.oz,
      dx: data.dx, dy: data.dy, dz: data.dz, weapon: shooter.weapon,
    });
  }
  ...
  _raycast(shooterId, data, wDef, damage) {
    ...
    for (const [id, p] of Object.entries(this.players)) {
      ...
      if (dist !== null && dist < closestDist) { closest = id; closestDist = dist; }
    }
    if (closest) {
  ...
  handlePickup(socketId, itemId) {
    ...
    if (item.type === 'weapon') { p.weapon = item.weapon; p.ammo = item.ammo; }
    else if (item.type === 'ammo') { p.ammo = Math.min(p.ammo + item.amount, WEAPONS[p.weapon]?.ammo * 3 || 90); }
    else if (item.type === 'heal') { p.hp = Math.min(PLAYER.MAX_HP, p.hp + item.amount); }
    else if (item.type === 'shield') { p.shield = Math.min(100, p.shield + item.amount); }
  ...
  _killPlayer(victimId, killerId, cause) {
    ...
    this.io.to(this.id).emit('playerDied', {
      id: victimId, username: victim.username, killerId, cause, aliveCount: this.aliveCount,
    });
    this._checkAlive();
  }
  ...
  async _endMatch(winner) {
    ...
    const results = Object.values(this.players).map(p => ({
      id: p.id, username: p.username, kills: p.kills, alive: p.alive,
    }));
    this.io.to(this.id).emit('matchEnd', {
      winner: winner ? { id: winner.id, username: winner.username } : null, results,
    });
    if (this.supabase) {
      try {
        await this.supabase.from('matches').insert({
          match_id: this.id, winner: winner?.username || null,
          player_count: results.length, results, ended_at: new Date().toISOString(),
        });
        for (const r of results) {
          await this.supabase.rpc('upsert_player_stats', {
            p_username: r.username, p_kills: r.kills, p_won: r.username === winner?.username,
          });
        }
  ...
  _publicPlayers() {
    return Object.fromEntries(
      Object.entries(this.players).map(([id, p]) => [id, {
        ...
        rotY: p.rotY, hp: p.hp, shield: p.shield, alive: p.alive, weapon: p.weapon,
      }])
    );
  }
